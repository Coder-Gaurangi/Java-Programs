...
{                                                  //start of class
	...
	{                                        //start of main method
		...
		n = sc.nextInt();                   //array size from user
		int arr[] = new int[n];
		System.out.println(" Enter the elements of the array : ");
		for(i = 0; i < n; i++)
		{
			arr[i] = sc.nextInt();     //array elements from user
		}
		System.out.println(" Original array : ");
		for(i = 0; i < n; i++)
		{
			System.out.print(" " + arr[i] + " "); 	//displaying input array
		}
		System.out.println();
		boolean flag = true;
		int temp = 0;
		for(i = 0; i < n - 1; i++)
		{                      //logic to implement zigzag fashion
			if(flag)
			{
				if(arr[i] > arr[i+1])
				{
